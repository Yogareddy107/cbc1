...
export default function GitHubCallbackCompletePage() {
    ...
    useEffect(() => {
        const checkSession = async () => {
            try {
                console.log('🔍 GitHub Callback: Starting session check');
                
                // Wait a bit for the session cookie to be set
                await new Promise(r => setTimeout(r, 2000));
                console.log('🔍 GitHub Callback: Waited 2 seconds, checking for session');
                
                // Try to get the current user
                const user = await account.get();
                console.log('✅ GitHub Callback: User found:', user);
                
                if (user) {
                    // Session exists, redirect to dashboard
                    setStatus('success');
                    console.log('✅ GitHub Callback: Redirecting to dashboard');
                ...
                setTimeout(() => {
                    router.push('/login?error=' + encodeURIComponent(`GitHub auth failed: ${errorMessage}`));
                }, 3000);
            }
        };
            ...
            <div className="flex h-screen w-full items-center justify-center bg-[#FFFDF6]">
                <div className="flex flex-col items-center gap-6 max-w-md">
                    ...
                    <div className="text-center space-y-2">
                        <p className="text-[#1A1A1A] font-bold text-lg">GitHub Authentication Failed</p>
    ...
    return (
        <div className="flex h-screen w-full items-center justify-center bg-[#FFFDF6]">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-[#1A1A1A]/60 font-medium">Completing GitHub authentication...</p>
